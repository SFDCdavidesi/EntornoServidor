  ...
  ?>
    ...
    foreach($res as $result){
        ...
        foreach($result as $k => $v){
           if ($i++==0){
          echo " <td scope='row'><a href='" . $_SERVER['PHP_SELF'] ."?id=" . $_REQUEST['id'] . "&editar=" . $result["usuario"] . "'>"  . Icono::edit() . "</a><button type=\"button\" class=\"btn btn-link\" data-bs-toggle=\"modal\" data-bs-target=\"#deleteUser\" onclick=\"ponUsuario('$v');\">" . Icono::delete() . " </button> </a></td>";
           }
            echo "<td>$v</td>";
        }
        echo "</tr>\r";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        Confirma que desea eliminar el usuario <span id="usuariomsg"></span>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-primary" id="confirmaEliminaUsuario" >Eliminar Usuario</button>
      </div>
    </div>
  </div>
</div>